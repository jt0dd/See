    ...
    img => {
      ...
      if (img.type != "error") {
        ...
        for (let percent = 100; percent >= 10; percent -= 5) {
          ...
          logger.log(
            "[Performance Logging] Average net run [25px x 25px] took: " +
              average +
              "ms with a total processing time of: " +
              total
          );
        }
        ...
        allTargets.forEach(target => {
          ...
          console.log("Math.round(target[0] * target[2]), Math.round(target[1] * target[2])", Math.round(target[0] * target[2]), Math.round(target[1] * target[2]))
          rawContext.rect(
